...
const static std::map<MacroActionFilter::Action, std::string> actionTypes = {
	...
	{MacroActionFilter::Action::COPY_BOUNDS_TO_SETTINGS,
...
static void CopyBoundsToSettings(obs_source_t *s_target,
				 const SceneItemSelection &boundSrc,
				 const SceneSelection &boundScene)
{
	auto sboundSIs = boundSrc.GetSceneItems(boundScene);
	if (sboundSIs.empty()) {
		return;
	}
	auto boundsJSON = GetSceneItemNoRotBounds(sboundSIs[0]);
	InsertDataToSourceSettings(s_target, "boundsNoRotInfo", boundsJSON);
}
...
static void performActionHelper(
	...
	const SceneItemSelection &boundSrcSI,
	const SceneSelection &boundSrcScene)
{
	...
	case MacroActionFilter::Action::SETTINGS_BUTTON:
		...
		break;
	case MacroActionFilter::Action::COPY_BOUNDS_TO_SETTINGS:
		CopyBoundsToSettings(source, boundSrcSI, boundSrcScene);
		break;
	default:
		break;
	}
...
bool MacroActionFilter::PerformAction()
{
	...
	for (const auto &filter : filters) {
		performActionHelper(_action, filter, _settingsInputMethod,
				    ...
				    _boundsSrcSource, _boundsSrcScene);
	}
	return true;
}
...
bool MacroActionFilter::Save(obs_data_t *obj) const
{
	...
	_boundsSrcSource.Save(obj, "boundsSelectionSource");
...
bool MacroActionFilter::Load(obs_data_t *obj)
{
	...
	_button.Load(obj);
	_boundsSrcScene.Load(obj, "boundsSelectionScene");
	_boundsSrcSource.Load(obj, "boundsSelectionSource");
	return true;
}
		  ...
		  "AdvSceneSwitcher.action.filter.copyBounds.entry.label")))
{
	...
	QWidget::connect(
		...
		SLOT(BoundsSrcSourceChanged(const SceneItemSelection &)));
	...
	QHBoxLayout *boundsSelectionLayout = new QHBoxLayout;
	std::unordered_map<std::string, QWidget *>
		widgetPlaceholdersBoundsSelection = {
			{"{{label}}", _boundsSrcLabel},
			{"{{boundsSrcScene}}", _boundsSrcScene},
			{"{{boundsSrcSource}}", _boundsSrcSource},
		};
	PlaceWidgets(obs_module_text(
			     "AdvSceneSwitcher.action.filter.copyBounds.entry"),
		     boundsSelectionLayout, widgetPlaceholdersBoundsSelection);
	...
	mainLayout->addLayout(boundsSelectionLayout);
...
void MacroActionFilterEdit::BoundsSrcSceneChanged(const SceneSelection &s)
{
	...
	auto lock = LockContext();
	_entryData->_boundsSrcScene = s;
}

void MacroActionFilterEdit::BoundsSrcSourceChanged(
	const SceneItemSelection &item)
{
	...
	auto lock = LockContext();
	_entryData->_boundsSrcSource = item;
	adjustSize();
	updateGeometry();
}
...
void MacroActionFilterEdit::SetWidgetVisibility()
{
	...
	bool showBoundsSrcSelection =
		_entryData->_action ==
		MacroActionFilter::Action::COPY_BOUNDS_TO_SETTINGS;
	_boundsSrcLabel->setVisible(showBoundsSrcSelection);
	_boundsSrcScene->setVisible(showBoundsSrcSelection);
	_boundsSrcSource->setVisible(showBoundsSrcSelection);
